...
from src.train_model import WineModelTrainer
...
def test_rank_clusters_to_profiles_orders_by_flavanoids_descending():
    ...
    mapping = WineModelTrainer.rank_clusters_to_profiles(labeled_df)
...
def test_train_writes_model_and_profile_with_three_segments(tmp_paths):
    ...
    expected_rows = len(WineModelTrainer.load_raw_data(config.DATA_PATH))

    trainer = WineModelTrainer(
        data_path=config.DATA_PATH, model_path=model_path, profile_path=profile_path
    )
